...
function plotAgainst(nameX, nameY)
    ...
    tab.add(char("Error in " + nameY),char("d"+nameY),...
        std( ydat(ETable.within(xdat, 0.15,5)) ) * ETable.within(xdat,0.15, 5)...
        + std( ydat(ETable.within(xdat, 0.15,10)) ) * ETable.within(xdat, 0.15,10)...
        + std( ydat(ETable.within(xdat, 0.15,15)) ) * ETable.within(xdat, 0.15,15)...
        + std( ydat(ETable.within(xdat, 0.15,20)) ) * ETable.within(xdat, 0.15,20)...
        + std( ydat(ETable.within(xdat, 0.15,25)) ) * ETable.within(xdat, 0.15,25)...
    );
    figure()
    hold on
